...
%------------------------------------------------------------------------
function seg = chanvese(I,init_mask,max_its,alpha,max_mean_diff, max_interior_points, display)
  ...
  for its = 1:max_its   % Note: no automatic convergence test
    ...
    if exist('max_mean_diff','var') && max_mean_diff < (v - u)
        break;
    end
    sum(size(upts))
     if exist('max_interior_points','var') && max_interior_points < sum(size(upts))
        break;
    end
